...
DIRECTIONS = ["north", "northeast", "east", "southeast", "south", "southwest",
              "west", "northwest"]
DIRECTION_ALIASES = {"n": "north", "ne": "northeast", "e": "east",
...
def list_available_commands(_):
    ...
    for command_help in command_helps:
        cprint(" - " + command_help, long_delay=.02)
...
class ModuleInterface(object):
    ...
    def __getattribute__(self, name):
        return station.modules[object.__getattribute__(self, "module_id")].__getattribute__(name)

    def __setattr__(self, name, value):
        station.modules[object.__getattribute__(self, "module_id")].__setattr__(name, value)

    def __eq__(self, other):
        # Two ModuleInterfaces are equal if they point to the same module
        if isinstance(other, self.__class__):
            return object.__getattribute__(self, "module_id") == object.__getattribute__(other, "module_id")
...
class Module():
    ...
    d_enter = Dialogue("You find yourself in the {title}.",
    ...
    def random(exclude: List[ModuleInterface] = []) -> ModuleInterface:
        ...
        return ModuleInterface(choice(list(filter(lambda mid: not ModuleInterface(mid) in exclude, station.modules.keys()))))
...
class Player(Entity, Hurtable):
    ...
    def print_stats(self):
        cprint(f"You have {self.flamethrower_fuel} flamethrower fuel and {self._health} health.")
...
class Telium(Entity):
    d_escape = Dialogue("In the corner of your eye you see an orange blob disappear through a door.",
...
class WorkerAlien(Entity, Hurtable):
    d_battle_start = Dialogue("{determiner} small orange alien scuttles across the floor and leaps towards you!",
    ...
    d_die = Dialogue("The alien falls lifeless to the floor, smouldering.",
                     "The alien lies charred on the floor.",
                     "The alien is blasted into a corner and stops moving.")
    d_player_kill = Dialogue("The alien chews off your leg and you die.",
                             "The alien kills you to death.")
    d_escape = Dialogue("Wounded, the alien disappears into a vent, escaping to another module.")
            ...
            while self.alive and player.alive:
                ...
                if use_fuel <= player.flamethrower_fuel:
                    ...
                    if not self.alive:
                        ...
                        cprint("It's not enough! The alien bites you viciously and moves to attack again.")
...
worker_aliens = [WorkerAlien(Module.random([player.module])) for _ in range(NUM_WORKER_ALIENS)]
...
while player.alive:
    ...
    if command_words:
        try:
            ...
            cprint(command.__doc__, long_delay=.02)
